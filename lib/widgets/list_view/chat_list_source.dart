...
class ChatListSource {
  ...
  late final List<ChatMessage> _list = [
    ChatMessage(content: '', sender: 0, group: group.id, id: 0, time: DateTime.now())
  ];
  bool _getting = false;
  bool noMore = false;
  ...
  void loadMore() async {
    ...
    if (res.isEmpty) {
      ...
      for (int i = res.length - 1; i >= 0; --i) {
        final _i = res[i];
        if (_list.first.time.difference(_i.time).inMinutes > 10) {
